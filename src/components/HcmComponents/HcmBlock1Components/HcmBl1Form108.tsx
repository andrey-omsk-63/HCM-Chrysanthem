    ...
    for (let i = 0; i < masStr.length; i++) {
      ...
      resStr.push(
        <Grid key={i} container sx={{ color: '#5B1080' }}>
          ...
          {TablStr(0, 1.0, ButtonLink(masStr[i].actor, ClickNik1), styleBl1Form09(brb))}
          {TablStr(0, 1.0, ButtonLink(masStr[i].avtor, ClickNik2), styleBl1Form09(brb))}
          {TablStr(0, 3.0, masStr[i].tema, styleBl1Form099(brb))}
          {TablStr(0, 1.0, ButtonLink(masStr[i].player, ClickNik3), styleBl1Form09(brb))}
  ...
  const HeaderTabl = () => {
    return (
      <Grid container sx={styleBl1Form07}>
        ...
        {TablStr(1, 1.0, 'Задача для', styleBl1Form08)}
